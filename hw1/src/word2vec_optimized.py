...
class Word2Vec(object):
  ...
  def train(self):
    ...
    while True:
      ...
      if step%10000==0:
        if step>0:
          with open('./submit_ans.'+str(epoch)+'.'+str(step),'wb') as f:
            embbeding1 = self._w_in.eval()
            for i in range(len(embedding1)):
              f.write(self._id2word[i])
              for q in range(len(embedding1[i])):
                f.write(' '+str(embedding1[i][q]))
              f.write('\n')
      '''
      if epoch != initial_epoch:
        break
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    for iterr in xrange(opts.epochs_to_train):
      model.train()  # Process one epoch
      with open('tmp/filter_word2vec.txt','wb') as f:
        embedding1 = model._w_in.eval()
        for i in range(len(embedding1)):
          f.write(model._id2word[i])
          for q in range(len(embedding1[i])):
            f.write(' '+str(embedding1[i][q]))
          f.write('\n')
      # print(model._w_in.eval())

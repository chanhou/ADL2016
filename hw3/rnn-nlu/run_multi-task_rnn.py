...
tf.app.flags.DEFINE_string("output", "/tmp", "Output directory.")
...
def read_data_test(source_path, max_size=None):
  """Read data from source and target files and put into buckets.
  ...
  with tf.gfile.GFile(source_path, mode="r") as source_file:
    ...
    while source and (not max_size or counter < max_size):
      ...
      if counter % 100000 == 0:
        ...
        sys.stdout.flush()
      source_ids = [int(x) for x in source.split()]
      target_ids = [-1 for x in source_ids]
      label_ids  = [-1]
#          target_ids.append(data_utils.EOS_ID)
      for bucket_id, (source_size, target_size) in enumerate(_buckets):
        if len(source_ids) < source_size and len(target_ids) < target_size:
...
def train():
  ...
  tag_vocab_path = ''
  label_vocab_path = ''
  in_seq_train, out_seq_train, label_train, in_seq_dev, out_seq_dev, label_dev, in_seq_test, vocab_path, tag_vocab_path, label_vocab_path = data_utils.prepare_multi_task_data(FLAGS.data_dir, FLAGS.in_vocab_size, FLAGS.out_vocab_size)

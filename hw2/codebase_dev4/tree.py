...
def loadTrees(dataSet='train'):
    ...
    """
    file = 'codebase_dev4/trees/rnn_eng/%s.treeFinal' % dataSet
...
def simplified_data(num_train, num_dev, num_test):
    ...
    pos_trees = loadTrees('train.posw2')#[t for t in trees if t.root.label==4]
    neg_trees = loadTrees('train.negw2')#[t for t in trees if t.root.label==0]
    test = loadTrees('test')#[t for t in trees if t.root.label==0]

    #filter extreme trees
    pos_trees_must = [t for t in pos_trees if t.root.label==4]
    neg_trees_must = [t for t in neg_trees if t.root.label==0]

    if len(pos_trees_must) < num_train:
        pos_trees_sub = [t for t in pos_trees if t.root.label==3]
        neg_trees_sub = [t for t in neg_trees if t.root.label==1]
        #pos_trees_sub = sorted(pos_trees_sub, key=lambda t: len(t.get_words()))
        #neg_trees_sub = sorted(neg_trees_sub, key=lambda t: len(t.get_words()))
        random.shuffle(pos_trees_sub)
        random.shuffle(neg_trees_sub)
    ...
    pos_trees = pos_trees_must + pos_trees_sub[:num_train-len(pos_trees_must)]
    neg_trees = neg_trees_must + neg_trees_sub[:num_train-len(neg_trees_must)]

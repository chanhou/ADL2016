...
class RNN_Model():
    ...
    def load_data(self):
        ...
        self.train_data, self.dev_data, self.test_data = tr.simplified_data(500,200,1000)
    ...
    def predict(self, trees, weights_path, get_loss = False):
        ...
        for i in xrange(int(math.ceil(len(trees)/float(RESET_AFTER)))):
            ...
            #with tf.Graph().as_default(),tf.Graph().device(_device_for_node),tf.Session(config=config) as sess:
                self.add_model_vars()
        ...
        while step < len(self.train_data):
            ...
            #with tf.Graph().as_default(),tf.Graph().device(_device_for_node),tf.Session(config=config) as sess:
                ...
                for _ in xrange(RESET_AFTER):
                    ...
                    logits = self.inference(tree,True)
                    root_label = tree.root.label
                    loss = self.loss(logits, [root_label])
    ...
    def train(self, verbose=True):
        ...
        for epoch in xrange(self.config.max_epochs):
            ...
            if not os.path.exists("./predict"):
                os.makedirs("./predict")
            test_preds, _ = self.predict(self.test_data, './weights/%s.temp'%self.config.model_name)
            with open('./predict/answer.txt.'+str(epoch),'w') as wwww:
                for i in test_preds:
                    wwww.write(str(i)+'\n')
...
def test_RNN():
    ...
    #with open('./model_loss.pickle','w') as f:
    #    pickle.dump(stats, f)


    print 'Test'
    print '=-=-='
    predictions, _ = model.predict(model.test_data, 'codebase_dev4/weights/%s.temp'%model.config.model_name)
    #labels = [t.root.label for t in model.test_data]
    #test_acc = np.equal(predictions, labels).mean()
    #print 'Test acc: {}'.format(test_acc)
    with open(sys.argv[1],'w') as wwww:
